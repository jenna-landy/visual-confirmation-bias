...
df <- df_study2 #df_study1, df_study2
```
...
report_lavaan_regs <- function(fit){
  ...
  out <- apply(pe, 1, function(row){
    ...
    sprintf(
      "%s → %s: b = %.2f, 95%% CI [%.2f, %.2f], SE = %.2f, z = %.2f, %s.",
...
```{r}
#simple regression
simple <- lm(post_evidence_guilt_belief ~ manipulation_condition + system_justification_GMC, df)
#post_evidence_guilt_belief #post_evidence_match #post_evidence_similarity
summary(simple) 
confint(simple)
```
...
#Y1: post_evidence_similarity (STUDY 1 ONLY)
#Y2: post_evidence_match (STUDY 1 ONLY)
#Y3: post_evidence_guilt_belief
...
```{r}
#sensitivity analyses for pilot studies 1 and 2

#study 1
#controlling for 8 predictors: visual search (key predictor), prior guilt beliefs, condition, sj, 4 covariates
#residual df: N = 253 - 8 - 1 = 244
p1 <- pwr.f2.test(
  u = 1, # focal predictor only
  v = 244,
  sig.level = 0.05,
  power = 0.80
)
...
#study 2
#controlling for 7 predictors: visual search (key predictor), prior guilt beliefs, condition, sj, 3 covariates
#residual df: N = 315 - 7 - 1 = 307
p2 <- pwr.f2.test(
  u = 1, # focal predictor only
  v = 307,
  sig.level = 0.05,
...
rsquared1 <- (fsquared1)/(1+fsquared1)
rsquared2 <- (fsquared2)/(1+fsquared2)
